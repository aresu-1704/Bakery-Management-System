...
namespace BakeryManagementSystem.Controllers
{
    public class QuanLyBanHang
    {
        ...
        public bool ThemSanPham(string maHH, double giaBan, bool coSan, ref int index)
        {
            if (maHoaDon == 0)
            {
                return false;
            ...
            {
                index = danhSachSP.FindIndex(p => p.MaHH == int.Parse(maHH));
                DanhSachSP[index].SoLuong += 1;
                if (coSan)
                {
                    DanhSachSP[index].SoLuongSanCo += 1;
                }
                return false;
            }
            DanhSachSP.Add(new ChiTietHoaDon(int.Parse(maHH), maHoaDon, giaBan));
        ...
        #region Xóa sản phẩm
        public bool XoaSanPham(int maHH, ref bool xoaSP)
        {
            bool coSan = false;
            int index = danhSachSP.FindIndex(p => p.MaHH == maHH);
            if (DanhSachSP[index].SoLuongSanCo < DanhSachSP[index].SoLuong)
            {
                DanhSachSP[index].SoLuong -= 1;
            }
            else
        ...
        #region Lưu chi tiết hóa đơn vào cơ sở Dữ liệu
        public async Task LuuCTHDAsync(int maNV, int maBan, int loaiHD, int maKH)
        {
            ...
            foreach (var sp in DanhSachSP)
            {
                ...
                if (sp.SoLuongSanCo > 0)
                {
                    tasks.Add(hangHoa.CapNhatSoLuongSanCoAsync(sp.MaHH, sp.SoLuongSanCo));
                }
        ...
        public async Task<DataTable> LayCTHDAsync(int maHD)
        {
